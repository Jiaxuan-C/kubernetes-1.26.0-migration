...
func NewMainKubelet(kubeCfg *kubeletconfiginternal.KubeletConfiguration,
	...
	// TODO(cjx.bupt): Init migrationManager.
	klet.migrationManager = migration.NewMigrationManager(klet.kubeClient, klet.podManager, klet.dispatchSyncPodMigrate, klet.rootDirectory)
...
type Kubelet struct {
	...
	// TODO(cjx.bupt): ADD migrationManager to handle Pods migration.
...
func (kl *Kubelet) syncPod(ctx context.Context, updateType kubetypes.SyncPodType, pod, mirrorPod *v1.Pod, podStatus *kubecontainer.PodStatus) (isTerminal bool, err error) {
	...
	// A biiiiiiiiiiiiiiiiiiiiiiiiiig bug!!!!!!!!!!!!!!!!
	//ctx := context.TODO()
	...
	// TODO(cjx.bupt): Migration logic in source kubelet. Handle SyncPodMigrate event.
	if updateType == kubetypes.SyncPodMigrate {
		klog.V(2).Info("Source Kubelet syncPod: Receive SyncPodMigrate event, preparing for checkpoint.")
		// mg是一个map成员，包含了path信息和pod中container名称列表
		mg, _ := kl.migrationManager.FindMigrationForPod(pod)
		// mg.Options()返回了迁移的选项，如是否需要checkpoint后停止容器、checkpoint pathDir、容器名，同时包含解锁HandleMigrationRequest
		kl.containerRuntime.CheckpointPod(ctx, pod, podStatus, mg.Options())

		// block the sync loop of the pod until the migration is finished.
		// 堵塞syncLoop直到迁移结束，锁住了，注意留意后边什么时候打开的，recv,等待协程向缓冲区写入数据才能解开阻塞
		mg.WaitUntilFinished()
		// 迁移结束后Terminate sourcePod？？？感觉可要可不要，交给上层处理也行.3月5日:最好还是加上，比较有用
...
// TODO(cjx.bupt): dispatchSyncPodMigrate dispatch the SyncPodMigrate event to syncPod.
func (kl *Kubelet) dispatchSyncPodMigrate(pod *v1.Pod) {
	start := kl.clock.Now()
	mirrorPod, _ := kl.podManager.GetMirrorPodByPod(pod)
...
func (kl *Kubelet) ListenAndServe(kubeCfg *kubeletconfiginternal.KubeletConfiguration, tlsOptions *server.TLSOptions,
	...
	server.ListenAndServeKubeletServer(kl, kl.resourceAnalyzer, kubeCfg, tlsOptions, auth, tp, kl.migrationManager)
}
