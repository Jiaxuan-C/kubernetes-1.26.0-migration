...
func (m *kubeGenericRuntimeManager) startContainer(ctx context.Context, podSandboxID string, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, podIP string, podIPs []string) (string, error) {
	...
	// TODO(cjx.bupt): Step 1 & 2: pull the image and create the container.
	containerID, containerConfig, message, err := m.createContainer(ctx, podSandboxID, podSandboxConfig, spec, pod, podStatus, pullSecrets, podIP, podIPs)
	if err != nil {
		return message, err
	}
...
func (m *kubeGenericRuntimeManager) startContainerTest(ctx context.Context, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, containerID string, containerConfig *runtimeapi.ContainerConfig) (string, error) {
	//(ctx context.Context, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, containerID string, containerConfig *runtimeapi.ContainerConfig, checkpointPath string)
	container := spec.container
	//// TODO(cjx.bupt): Step 1 & 2: pull the image and create the container.
	//containerID, containerConfig, message, err := m.createContainer(ctx, podSandboxID, podSandboxConfig, spec, pod, podStatus, pullSecrets, podIP, podIPs)
	//if err != nil {
...
// createContainer creates a container and returns a message indicates why it is failed on error.
// It creates the container through the following steps:
// * pull the image
// * create the container
func (m *kubeGenericRuntimeManager) createContainer(ctx context.Context, podSandboxID string, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, podIP string, podIPs []string) (containerID string, containerConfig *runtimeapi.ContainerConfig, message string, err error) {
	container := spec.container
	...
	if err != nil {
		...
		return "", nil, s.Message(), ErrPreCreateHook
	}
...
// TODO(cjx.bupt):
// restoreContainer restores containers from checkpoint in migration and returns a message indicates why it is failed on error.
// It restores the container through the following steps:
// * pull container snapshot
// * restore the container
// * run the post start lifecycle hooks (if applicable)
func (m *kubeGenericRuntimeManager) restoreContainer(ctx context.Context, podSandboxConfig *runtimeapi.PodSandboxConfig, spec *startSpec, pod *v1.Pod, containerID string, containerConfig *runtimeapi.ContainerConfig, checkpointPath string) (string, error) {
	...
	for retryCount > 0 && err != nil {
		retryCount--
		restoreContainerBeginTime := time.Now().UnixMilli()
		klog.Infof("===================================================Restore Begin time: %d===================================================", int64(restoreContainerBeginTime))
		err = m.runtimeService.RestoreContainer(ctx, &runtimeapi.RestoreContainerRequest{
			ContainerId: containerID,
			Location:    checkpointPath,
			ClonePodId:  pod.Labels["CloneSourcePod"],
		})
		restoreContainerEndTime := time.Now().UnixMilli()
		restoreTime := restoreContainerEndTime - restoreContainerBeginTime
		klog.Infof("===================================================Restore time: %d===================================================", int64(restoreTime))
		klog.Infof("===================================================Restore End time: %d===================================================", int64(restoreContainerEndTime))
	}
	if err != nil {
		s, _ := grpcstatus.FromError(err)
		m.recordContainerEvent(pod, container, containerID, v1.EventTypeWarning, events.FailedToStartContainer, "Error: %v", s.Message())
...
// CheckpointContainers dumps source Pods' containers in path and returns a message indicates why it is failed on error. TODO(cjx.bupt):
func (m *kubeGenericRuntimeManager) CheckpointContainers(ctx context.Context, container *v1.Container, podStatus *kubecontainer.PodStatus, options *kubecontainer.MigratePodOptions, pod *v1.Pod, sftpClient *sftp.Client, sshClient *ssh.Client, preDumpTimes int) error {
	klog.Infof("Checkpointing container %v.", container.Name)
	containerStatus := podStatus.FindContainerStatusByName(container.Name)
	...
	migrationWorkPath := path.Join(options.CheckpointsDir, container.Name)
	fullCheckpointPath := path.Join(migrationWorkPath, "/fullCheckDir")
	// full check logic.
	fullCheckpointParentPath := "./../preDump" + strconv.Itoa(preDumpTimes)
	err := errors.New("")
	dumpContainerBeginTime := time.Now().UnixMilli()
	if preDumpTimes != 0 {
		err = m.runtimeService.CheckpointContainer(ctx, &runtimeapi.CheckpointContainerRequest{
			ContainerId: containerStatus.ID.ID,
			Location:    fullCheckpointPath,
			ParentPath:  fullCheckpointParentPath,
		})
	} else {
		err = m.runtimeService.CheckpointContainer(ctx, &runtimeapi.CheckpointContainerRequest{
			ContainerId: containerStatus.ID.ID,
	...
	dumpContainerEndTime := time.Now().UnixMilli()
	dumpTime := dumpContainerEndTime - dumpContainerBeginTime
	klog.Infof("===================================================Dump Time: %d===================================================", dumpTime)
	// tar and upload container rw layer files.
	mergePath := path.Join("/run/containerd/io.containerd.runtime.v2.task/k8s.io", containerStatus.ID.ID, "rootfs")
	...
	for _, line := range strings.Split(string(mounts), "\n") {
		if strings.HasPrefix(line, "overlay "+mergePath) {
			rwPath = strings.Split(strings.Split(strings.Split(line, " ")[3], "upperdir=")[1], ",")[0]
			break
		}
	}
	if err := migration.TarContainerRW(rwPath, migrationWorkPath); err != nil {
		return err
	}
	// upload full checkpoint files to remote node.
	err = migration.UploadDumpFile(sftpClient, fullCheckpointPath, fullCheckpointPath)
	if err != nil {
		return err
	}
	if err = migration.UploadDumpFile(sftpClient, path.Join(migrationWorkPath, "fs.tar"), path.Join(migrationWorkPath, "fs.tar")); err != nil {
		return err
	}
	CheckpointContainersEndTime := time.Now().UnixMilli()
	CheckpointContainersDuringTime := CheckpointContainersEndTime - dumpContainerBeginTime
	klog.Infof("===================================================CheckpointContainersEndTime: %d===================================================", CheckpointContainersEndTime)
	klog.Infof("===================================================CheckpointContainersDuringTime: %d===================================================", int64(CheckpointContainersDuringTime))
	return err
}

func (m *kubeGenericRuntimeManager) PreDumpContainers(container *v1.Container, podStatus *kubecontainer.PodStatus, options *kubecontainer.MigratePodOptions, pod *v1.Pod, sftpClient *sftp.Client, sshClient *ssh.Client) (int, error) {
	klog.Infof("Pre-dump container %v.", container.Name)
	containerStatus := podStatus.FindContainerStatusByName(container.Name)
	// If a container isn't running, it can't be live-migrated.
	if containerStatus == nil || containerStatus.State != kubecontainer.ContainerStateRunning {
		return 0, nil
	}
	preDumpLocation := path.Join(options.CheckpointsDir, container.Name)
	// pre-Dump logic.
	klog.Infof("Pre-dumping container %v.", container.Name)
	topPreDumpTimes := 15
	preDumpTimes, err := migration.RuncPreDump(topPreDumpTimes, preDumpLocation, containerStatus.ID.ID)
	if err != nil {
		klog.ErrorS(err, "failed to pre-dump container:", &container.Name)
		return 0, err
	}
	// upload pre-dump files to remote node.
	err = migration.UploadDumpFile(sftpClient, preDumpLocation, preDumpLocation)
	if err != nil {
		return 0, err
	}

	return preDumpTimes, nil
}
