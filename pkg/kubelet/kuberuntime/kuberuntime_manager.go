...
type kubeGenericRuntimeManager struct {
	...
	// TODO(cjx.bupt) Migration manager
...
func (m *kubeGenericRuntimeManager) SyncPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff) (result kubecontainer.PodSyncResult) {
	...
	// TODO(cjx.bupt): creates and restores containers
	if pod.Labels["CloneSourcePod"] != "" && len(podContainerChanges.ContainersToStart) == len(pod.Spec.Containers) {
		klog.Info("Migrator: Creating clone containers.")
		containerIDList := make([]string, len(podContainerChanges.ContainersToStart))
		containerConfigList := make([]*runtimeapi.ContainerConfig, len(podContainerChanges.ContainersToStart))
		startContainerResultList := make([]*kubecontainer.SyncResult, len(podContainerChanges.ContainersToStart))
		metricLabel := metrics.Container
		// Create containers in target node
		for _, index := range podContainerChanges.ContainersToStart {
			spec := containerStartSpec(&pod.Spec.Containers[index])
			startContainerResultList[index] = kubecontainer.NewSyncResult(kubecontainer.StartContainer, spec.container.Name)
			result.AddSyncResult(startContainerResultList[index])

			// TODO (cjx.bupt):
			isInBackOff, msgk, errk := m.doBackOff(pod, spec.container, podStatus, backOff)
			if isInBackOff {
				startContainerResultList[index].Fail(errk, msgk)
				klog.V(4).InfoS("Backing Off restarting container in pod", "containerType", "container", "container", spec.container, "pod", klog.KObj(pod))
				return
			}
			...
			var msg string
			var err error
			if containerIDList[index], containerConfigList[index], msg, err = m.createContainer(ctx, podSandboxID, podSandboxConfig, spec, pod, podStatus, pullSecrets, podIP, podIPs); err != nil {
				startContainerResultList[index].Fail(err, msg)
				// known errors that are logged in other places are logged at higher levels here to avoid
				// repetitive log spam
				switch {
				case err == images.ErrImagePullBackOff:
		...
		// TODO (cjx.bupt): Send GET request to source node.
		...
		for _, idx := range podContainerChanges.ContainersToStart {
			...
			go func(idx int) {
				...
				container := &pod.Spec.Containers[idx]
				migContainerCheckPath, ok := migResult.Containers[container.Name]
				if !ok {
				...
				fullCheckpointPath := path.Join(migContainerCheckPath, "fullCheckDir")
				if msg, err := m.restoreContainer(ctx, podSandboxConfig, containerStartSpec(&pod.Spec.Containers[idx]), pod, containerIDList[idx], containerConfigList[idx], fullCheckpointPath); err != nil {
					// TODO(cjx.bupt):
					metrics.StartedContainersErrorsTotal.WithLabelValues(metricLabel, err.Error()).Inc()
					if sc.HasWindowsHostProcessRequest(pod, spec.container) {
						metrics.StartedHostProcessContainersErrorsTotal.WithLabelValues(metricLabel, err.Error()).Inc()
					...
					startContainerResultList[idx].Fail(err, msg)
...
// CheckpointPod  TODO(cjx.bupt)
func (m *kubeGenericRuntimeManager) CheckpointPod(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, options *kubecontainer.MigratePodOptions) {
	klog.V(2).Info("Pre-copy Pod %v for migration. %v", pod.Name, options)
	containerToPreDumpTimes := make(map[string]int)
	// Init sftp config
	sftpClient, sshClient, err := migration.SftpInit("root", "root", pod.Labels["MigratingTo"])
	if err != nil {
		klog.ErrorS(err, "failed to init sftpClient")
		return
	}
	defer sshClient.Close()
	defer sftpClient.Close()
	// The switch of pre-copy.
	OPEN_PREDUMP_SWITCH := 1
	if OPEN_PREDUMP_SWITCH == 1 {
		wg1 := sync.WaitGroup{}
		wg1.Add(len(options.Containers))
		// 两层循环校验container
		for _, c := range options.Containers {
			for _, container := range pod.Spec.Containers {
				if container.Name == c {
					go func() {
						// 进去pre-copy
						preDumpTimes, err := m.PreDumpContainers(&container, podStatus, options, pod, sftpClient, sshClient)
						if err != nil {
							klog.ErrorS(err, "failed to pre-dump container:", &container.Name)
						}
						containerToPreDumpTimes[container.Name] = preDumpTimes
						wg1.Done()
					}()
					break
				}
	...
	wg2 := sync.WaitGroup{}
	wg2.Add(len(options.Containers))
	// 两层循环校验container
	for _, c := range options.Containers {
		for _, container := range pod.Spec.Containers {
			if container.Name == c {
				go func() {
					// 进去checkpoint
					klog.Infof("===================================================Downtime Begin: %d===================================================", time.Now().UnixMilli())
					if OPEN_PREDUMP_SWITCH == 1 {
						err := m.CheckpointContainers(ctx, &container, podStatus, options, pod, sftpClient, sshClient, containerToPreDumpTimes[container.Name])
						if err != nil {
							klog.ErrorS(err, "failed to checkpoint container:", &container.Name)
						}
					} else {
						err := m.CheckpointContainers(ctx, &container, podStatus, options, pod, sftpClient, sshClient, 0)
						if err != nil {
							klog.ErrorS(err, "failed to checkpoint container:", &container.Name)
						}
					}
