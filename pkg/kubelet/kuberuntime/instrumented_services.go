...
func (in instrumentedRuntimeService) RestoreContainer(ctx context.Context, options *runtimeapi.RestoreContainerRequest) error {
	const operation = "restore_container"
	defer recordOperation(operation, time.Now())

	err := in.service.RestoreContainer(ctx, options)
	recordError(operation, err)
	return err
}
