...
func ListenAndServeKubeletServer(
	...
	handler := NewServer(host, resourceAnalyzer, auth, tp, kubeCfg, migrationManager)
	s := &http.Server{
...
func ListenAndServeKubeletReadOnlyServer(
	...
	s := NewServer(host, resourceAnalyzer, nil, oteltrace.NewNoopTracerProvider(), nil, nil)
...
func NewServer(
	...
	server.InstallDefaultHandlers(migrationManager)
...
func (s *Server) InstallDefaultHandlers(migrationManager migration.Manager) {
	...
	// TODO(cjx.bupt): add migration endpoint
	...
	ws.Route(ws.GET("/{podUID}").
		To(migrationManager.HandleTargetKubeletRequest).
